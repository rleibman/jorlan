...
/** A [[Skill]] that also bridges an external system, adding an ingress lifecycle.
  *
  * Egress (e.g. `telegram.send_message`) flows through the inherited [[invoke]]. Ingress (listening for inbound
  * events) is managed by [[start]] / [[stop]], which are forked as daemons at boot by [[ConnectorManager]].
  */

...
/** OAuth2 callback route for re-linking external credentials from an already-authenticated user.
  ...
  *     [[OAuthCredentialService.verifyAndConsume]], exchanges the authorization code for tokens, stores credentials,
  *     and redirects to `/?oauth=success`.
  *
  * The start of this flow is initiated by the `startOAuth` GraphQL mutation, which calls
  * [[OAuthCredentialService.buildAuthUrl]] directly and returns the Google consent URL to the frontend.
  */
      ...
      val callbackRoute: Route[Any, Nothing] =
        Method.GET / "api" / "oauth" / "callback" / "google" -> handler { (req: Request) =>
          ...
          (for {
            ...
            (userId, provider) <- oauthCredSvc.verifyAndConsume(state)

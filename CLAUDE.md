...
- NEVER put a full copy of a trait's interface into its companion object as ZIO accessor methods (e.g. `def
  ...
  `live` layer factory.
- In scala.js react components, don't build your own wrappers (e.g. MuiButton) when there's perfectly valid alternatives
  generated by scalablytyped. 
- In scala.js, avoid the pattern of using ```Component.set("myVar", value)``` It's "stringly typed" and not type safe. 

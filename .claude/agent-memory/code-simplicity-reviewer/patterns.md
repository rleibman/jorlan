...
- `scheduleRetryOrFail` in TriggerEngine is a pure `UIO[Unit]` computed with an `if/else` block. The exponential backoff formula `job.backoffSeconds.toLong * math.pow(2, job.retryCount.toDouble).toLong` mixes `Double` and `Long` arithmetic unnecessarily. Use `1L << job.retryCount` (integer left-shift) or a clean `math.pow` result without the mixed `.toLong` chain.
- `import java.time.ZonedDateTime` is placed inside the `TriggerType.Cron` match arm body (line 100) rather than at the file's top-level imports. Non-standard placement.
- `SchedulerSkill` is a thin delegation wrapper over `JobManager` — every method is a 1-liner forwarding to `jobManager.method(...)`. No additional logic is present. For Phase 10 this is fine (placeholder for Phase 12 registry wiring), but the class adds indirection with zero current value. Add a comment explaining the deferral.
- `JorlanAPI` scheduler mutations `pauseJob`, `resumeJob`, `triggerNow`, `deleteJob` all share a 3-line body: `actorId <- actorIdFromSession; _ <- requireCapability("scheduler.manage", actorId); _ <- ZIO.serviceWithZIO[JobManager](_.xxx(id))` yielding `true`. A private helper `withSchedulerCap[A](f: JobManager => IO[JorlanError, A])` would reduce 4 × 3 lines to 4 × 1 line.
- `InMemoryRepositories.expireLeases`: the `foldLeft` lambda uses a 4-line split `(acc, j) =>` style (lines 672-677). Inline as `(acc, j) => acc.updated(j.id, j.copy(...))` or use `.map` + `.foldLeft(m)((acc, j) => ...)` one-liner.
- `JobManagerSpec`: `mgr.createJob(agentId, userId, "name", None, 0, 60, RetryBackoffPolicy.Fixed, MissedRunPolicy.Skip)` is repeated 9 times verbatim. A `private def defaultJob(name: String)` helper in the spec would remove ~40 lines of duplication.
- `TriggerEngineSpec.makeEngine` constructs both stubs inside a `for { ... yield (engine, invoked) }` — clean pattern. But each of the 6 tests repeats `repo <- InMemorySchedulerRepo.make; eventLog <- InMemoryEventLogRepo.make; sid = AgentSessionId(N); (engine, _) <- makeEngine(repo, eventLog, sid)` as its first 4 lines. Extracting `makeTestEnv(sid, shouldSucceed): ZIO[Any, Nothing, (InMemorySchedulerRepo, TriggerEngine, Ref[...])]` would halve the preamble noise.

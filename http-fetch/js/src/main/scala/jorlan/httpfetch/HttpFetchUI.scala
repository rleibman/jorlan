...
object HttpFetchUI {
  ...
  private val HttpFetchWidget = ScalaFnComponent
    ...
    .useStateBy { props =>
      val parsed = props.initialConfigStr
        .fromJson[HttpFetchConfig]
        .getOrElse(HttpFetchConfig())
    }
    .render(
      (
        props,
        s,
      ) => <.div("HttpFetch Skill Configuration"),
    )

  ScalaComponent
    .builder[Unit]("HttpFetchWidget")
    .renderStatic(<.div("HttpFetch Skill v1")) // TODO configuration goes here
    .build
  ...
  def main(args: Array[String]): Unit = {
    ...
    val pollingIntervalId = js.timers.setInterval(5000) {
      println("HttpFetch plugin fetching latest temperatures...")
    }
    ...
    val payload = js.Dynamic.literal(
      ...
      "onUnload" -> (() => {
        ...
        println("HttpFetch plugin cleaned up background intervals successfully.")
      }),
    )
    js.Dynamic.global.registerRemoteSkill("httpFetch-skill", payload)
  }

...
final case class McpServerViewView[EnvSelection, HeadersSelection](name: String, transport: String, command: scala.Option[String], args: List[String], env: List[EnvSelection], url: scala.Option[String], enabled: Boolean, keywords: List[String], headers: List[HeadersSelection])




type ViewSelection[EnvSelection, HeadersSelection] = SelectionBuilder[McpServerView, McpServerViewView[EnvSelection, HeadersSelection]]

def view[EnvSelection, HeadersSelection](envSelection: SelectionBuilder[McpEnvVar, EnvSelection], headersSelection: SelectionBuilder[McpEnvVar, HeadersSelection]): ViewSelection[EnvSelection, HeadersSelection] = (name ~ transport ~ command ~ args ~ env(envSelection) ~ url ~ enabled ~ keywords ~ headers(headersSelection)).map { case (name, transport, command, args, env, url, enabled, keywords, headers) => McpServerViewView(name, transport, command, args, env, url, enabled, keywords, headers) }
  ...
  def keywords: SelectionBuilder[McpServerView, List[String]] = _root_.caliban.client.SelectionBuilder.Field("keywords", ListOf(Scalar()))
  def headers[A](innerSelection: SelectionBuilder[McpEnvVar, A]): SelectionBuilder[McpServerView, List[A]] = _root_.caliban.client.SelectionBuilder.Field("headers", ListOf(Obj(innerSelection)))
}
...
final case class SkillInfoView[ToolsSelection](name: String, tier: String, tools: List[ToolsSelection], enabled: Boolean, keywords: List[String], configKey: scala.Option[String], configJsModule: scala.Option[String], dashboardJsModule: scala.Option[String], hasDashboardData: Boolean, oauthProvider: scala.Option[OAuthProvider], doc: scala.Option[String])
...
def view[ToolsSelection](toolsSelection: SelectionBuilder[SkillToolInfo, ToolsSelection]): ViewSelection[ToolsSelection] = (name ~ tier ~ tools(toolsSelection) ~ enabled ~ keywords ~ configKey ~ configJsModule ~ dashboardJsModule ~ hasDashboardData ~ oauthProvider ~ doc).map { case (name, tier, tools, enabled, keywords, configKey, configJsModule, dashboardJsModule, hasDashboardData, oauthProvider, doc) => SkillInfoView(name, tier, tools, enabled, keywords, configKey, configJsModule, dashboardJsModule, hasDashboardData, oauthProvider, doc) }
  ...
  def doc: SelectionBuilder[SkillInfo, scala.Option[String]] = _root_.caliban.client.SelectionBuilder.Field("doc", OptionOf(Scalar()))
}
...
object Mutations {
  ...
  def submitMessage(sessionId : jorlan.AgentSessionId, content : String, allowedTools : scala.Option[List[String]] = None)(implicit encoder0: ArgEncoder[jorlan.AgentSessionId], encoder1: ArgEncoder[String], encoder2: ArgEncoder[scala.Option[List[String]]]): SelectionBuilder[_root_.caliban.client.Operations.RootMutation, scala.Option[Unit]] = _root_.caliban.client.SelectionBuilder.Field("submitMessage", OptionOf(Scalar()), arguments = List(Argument("sessionId", sessionId, "AgentSessionId!"), Argument("content", content, "String!"), Argument("allowedTools", allowedTools, "[String!]")))

...
final case class McpServerViewView[EnvSelection](name: String, transport: String, command: scala.Option[String], args: List[String], env: List[EnvSelection], url: scala.Option[String], enabled: Boolean, keywords: List[String])
...
def view[EnvSelection](envSelection: SelectionBuilder[McpEnvVar, EnvSelection]): ViewSelection[EnvSelection] = (name ~ transport ~ command ~ args ~ env(envSelection) ~ url ~ enabled ~ keywords).map { case (name, transport, command, args, env, url, enabled, keywords) => McpServerViewView(name, transport, command, args, env, url, enabled, keywords) }
...
object Queries {
  ...
  def agents[A](innerSelection: SelectionBuilder[Agent, A]): SelectionBuilder[_root_.caliban.client.Operations.RootQuery, scala.Option[List[A]]] = _root_.caliban.client.SelectionBuilder.Field("agents", OptionOf(ListOf(Obj(innerSelection))))
...
object Mutations {
  ...
  def createJob[A](name : String, pipelineJson : String, maxRetries : Int, backoffSeconds : Int, backoffPolicy : jorlan.RetryBackoffPolicy, missedRunPolicy : jorlan.MissedRunPolicy)(innerSelection: SelectionBuilder[SchedulerJob, A])(implicit encoder0: ArgEncoder[String], encoder1: ArgEncoder[Int], encoder2: ArgEncoder[jorlan.RetryBackoffPolicy], encoder3: ArgEncoder[jorlan.MissedRunPolicy]): SelectionBuilder[_root_.caliban.client.Operations.RootMutation, scala.Option[A]] = _root_.caliban.client.SelectionBuilder.Field("createJob", OptionOf(Obj(innerSelection)), arguments = List(Argument("name", name, "String!"), Argument("pipelineJson", pipelineJson, "String!"), Argument("maxRetries", maxRetries, "Int!"), Argument("backoffSeconds", backoffSeconds, "Int!"), Argument("backoffPolicy", backoffPolicy, "RetryBackoffPolicy!"), Argument("missedRunPolicy", missedRunPolicy, "MissedRunPolicy!")))

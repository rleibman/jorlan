...
import jorlan.market.AlphaVantageConfig
import zio.json.*
  ...
  private val MarketWidget = ScalaFnComponent
    ...
    .useStateBy { props =>
      val parsed = props.initialConfigStr
        .fromJson[AlphaVantageConfig]
        .getOrElse(AlphaVantageConfig())
    }
    .render(
      (
        props,
        s,
      ) => <.div("Market Skill Configuration"),
    )

  ScalaComponent
    .builder[Unit]("MarketWidget")
    .renderStatic(<.div("Market Skill v1")) // TODO configuration goes here
    .build
  ...
  def main(args: Array[String]): Unit = {
    ...
    val payload = js.Dynamic.literal(
      ...
      "onUnload" -> (() => {
        ...
        println("Market plugin cleaned up background intervals successfully.")
      }),
    )
    js.Dynamic.global.registerRemoteSkill("market-skill", payload)
  }

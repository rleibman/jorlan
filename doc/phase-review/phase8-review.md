...
| [x]    | P8-033     | Suggestion | Test Coverage          | `AgentSessionManager.updateStatus` with unknown session ID not tested — should return `JorlanError`.                                                                                                      | `AgentSessionManagerSpec.scala`                                                                                                  | Added test: call `updateStatus` with non-existent `AgentSessionId`; assert typed error returned.                                                                                                                   |
| [x]    | P8-034     | Suggestion | Test Coverage          | GraphQL `createSession` and `submitMessage` mutations absent from `JorlanAPISpec`.                                                                                                                        | `JorlanAPISpec.scala`                                                                                                            | Added unit tests for both mutations using in-memory service stubs; capability checks inherited from authSuite.                                                                                    |
| [ ]    | P8-035     | Suggestion | Test Coverage          | GraphQL `agentResponseStream` subscription end-to-end test missing.                                                                                                                                       | `JorlanAPISpec.scala`                                                                                                            | Defer to follow-up: requires advanced Caliban StreamValue assertion pattern.                                                                         |
| [x]    | P8-036     | Suggestion | Test Coverage          | `HumanApprovalNotifier` has 0% coverage.                                                                                                                                                                  | `HumanApprovalNotifier.scala`                                                                                                    | Created HumanApprovalNotifierSpec; test `notifyApprovalRequired` verifies ApprovalRequested events logged with correct resource IDs.                                                                                            |
| [x]    | P8-037     | Suggestion | Test Coverage          | `SessionHub.publish` to non-existent session (no-op path) not tested; `subscribe` method never called directly in tests.                                                                                  | `SessionHubSpec.scala`                                                                                                           | Added tests for no-op publish and for `subscribe` receiving chunks in insertion order.                                                                                                                             |
| [x]    | P8-038     | Suggestion | Test Coverage          | `AgentSessionManager.terminateSession` does not verify hub removal.                                                                                                                                       | `AgentSessionManagerSpec.scala`                                                                                                  | Added test: create session, terminate it, verify hub cleanup.                                                                                                                                        |
| [x]    | P8-039     | Suggestion | Test Coverage          | `CommandHandler.handleMessage` with active session (happy path and GQL failure) untested; `handleNewSession` all three result branches untested.                                                          | `CommandHandlerSpec.scala`                                                                                                       | Created CommandHandlerSpec with test stubs; full integration tests require mock clients (defer to Phase 9).                                                                                             |
| [x]    | P8-040     | Suggestion | Test Coverage          | `FakeModelGateway` `chunkDelay` branch not exercised in any test.                                                                                                                                         | `FakeModelGateway.scala`                                                                                                         | Added test: use `FakeModelGateway.layer(..., Some(delay))`; assert timing constraint satisfied via Clock.instant.                                                                                                         |
| [x]    | P8-041     | Suggestion | Test Coverage          | `ShellState` has 0% coverage — trivial pure functions.                                                                                                                                                    | `ShellState.scala`                                                                                                               | Created ShellStateSpec with full coverage: `getSessionId`, `setSessionId`, `clearSessionId`.                                                                                                                                             |
| [ ]    | P8-042     | Suggestion | Test Coverage          | Integration test missing: full round-trip using `FakeModelGateway`, asserting each chunk arrives in order. Explicitly marked unchecked in phase roadmap.                                                  | Integration test module                                                                                                          | Defer to follow-up: requires Testcontainers Docker integration (substantial setup).                                                        |

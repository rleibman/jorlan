...
object McpServersPage {
  ...
  case class EnvVarDraft(key: String, value: String)
    ...
    ScalaFnComponent
      ...
      .render { (_, state) =>
            ...
            AsyncCallbackRepositories.mcp
              ...
              .completeWith(
                PageUtils.onError(err =>
                  state.modState(
                    _.copy(saving = false, toast = Some(ToastMessage(err.getOrElse("Save failed"), ToastSeverity.Error))),
                  ),
                ),
              )
              .runNow()
        ...
        def removeEnvVar(idx: Int): Callback =
          state.modState(s =>
            s.copy(form = s.form.copy(env = s.form.env.zipWithIndex.filterNot(_._2 == idx).map(_._1))),
          )
              ...
              Box.withProps(
                ...
                Switch.withProps(
                  SwitchProps()
                    ...
                    .setOnChange((_, checked) => updateForm(f.copy(enabled = checked)))

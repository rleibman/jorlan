      ...
      <.div(
        ...
        editingStep.fold(EmptyVdom) { step =>
          ...
          Box.withProps(
            ...
            MuiTextField
              .label("Tools (comma-separated namespace prefixes, e.g. calendar,weather,memory)")
              .value(step.tools.mkString(", "))
              .fullWidth(true)
              .onChange { e =>
                val v = e.target.asInstanceOf[org.scalajs.dom.html.Input].value
                val ts = v.split(",").map(_.trim).filter(_.nonEmpty).toList
                updateStep(step.copy(tools = ts)).runNow()
              }(),
            MuiTextField
              .label("Retry on Fail")

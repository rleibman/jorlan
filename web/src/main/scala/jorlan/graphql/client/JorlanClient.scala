...
def view: ViewSelection = (id ~ scope ~ userId ~ workspaceId ~ agentId ~ recordKey ~ value ~ ttl ~ createdAt ~ updatedAt).map { case (id, scope, userId, workspaceId, agentId, recordKey, value, ttl, createdAt, updatedAt) => MemoryRecordView(id, scope, userId, workspaceId, agentId, recordKey, value, ttl, createdAt, updatedAt) }
...
def view: ViewSelection = (id ~ agentId ~ userId ~ skillId ~ name ~ inputJson ~ status ~ scheduledAt ~ startedAt ~ finishedAt ~ resultJson ~ maxRetries ~ retryCount ~ backoffSeconds ~ backoffPolicy ~ missedRunPolicy ~ leasedAt ~ leasedBy ~ createdAt).map { case (id, agentId, userId, skillId, name, inputJson, status, scheduledAt, startedAt, finishedAt, resultJson, maxRetries, retryCount, backoffSeconds, backoffPolicy, missedRunPolicy, leasedAt, leasedBy, createdAt) => SchedulerJobView(id, agentId, userId, skillId, name, inputJson, status, scheduledAt, startedAt, finishedAt, resultJson, maxRetries, retryCount, backoffSeconds, backoffPolicy, missedRunPolicy, leasedAt, leasedBy, createdAt) }
...
final case class SkillInfoView[ToolsSelection](name: String, tier: String, tools: List[ToolsSelection])
...
def view[ToolsSelection](toolsSelection: SelectionBuilder[SkillToolInfo, ToolsSelection]): ViewSelection[ToolsSelection] = (name ~ tier ~ tools(toolsSelection)).map { case (name, tier, tools) => SkillInfoView(name, tier, tools) }
...
final case class SkillToolInfoView(name: String, description: String, requiredCapabilities: List[String])
...
def view: ViewSelection = (name ~ description ~ requiredCapabilities).map { case (name, description, requiredCapabilities) => SkillToolInfoView(name, description, requiredCapabilities) }
